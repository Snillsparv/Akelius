...
2. **Anonymiseringen bevaras**: Mr X → herr X, Miss X → fröken X, Mrs X → fru X, King X → kung X, Captain X → kapten X, Brother X → broder X, Movement X → rörelsen X, Period X → perioden X, Place X → platsen X, Trade X → handeln X, Emperor X → kejsar X, Admiral X → amiral X, Road X → vägen X, Master X →
